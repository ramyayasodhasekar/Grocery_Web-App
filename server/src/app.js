...
app.get('/userprofile/:id', async (req, res) => {
    try {
      ...
      const user = await models.Users.findById(userId); // Fetch user from MongoDB using the ID
...
app.post('/register', async (req, res) => {
    try {
        ...
        return res.status(201).json({ message: 'Registration successful', user: userCreated });
    } catch (error) {
